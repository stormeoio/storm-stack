...
const CATALOG_REGISTRY: Omit<CatalogEntry, "status">[] = [
  { id: "@stormeoio/auth", shortName: "auth", name: "Auth", description: "Email/password authentication with JWT cookies, RBAC, and multi-tenant support", tags: ["auth", "security", "rbac", "multi-tenant"], pricing: "free", requires: [], envVars: { SESSION_SECRET: { description: "JWT signing secret (min 32 chars)", required: true, example: "change-me-32-chars-min" } }, version: PACKAGE_VERSION, category: "security" },
  { id: "@stormeoio/auth-social", shortName: "auth-social", name: "Auth Social", description: "OAuth2 authentication with Google, GitHub, and GitLab", tags: ["auth", "oauth", "google", "github"], pricing: "free", requires: ["@stormeoio/auth"], envVars: { GOOGLE_CLIENT_ID: { description: "Google OAuth client ID", required: false }, GITHUB_CLIENT_ID: { description: "GitHub OAuth client ID", required: false } }, version: PACKAGE_VERSION, category: "security" },
  { id: "@stormeoio/consent", shortName: "consent", name: "Consentement", description: "Préférences de consentement et cookies par utilisateur", tags: ["consent", "cookies", "privacy", "rgpd", "gdpr"], pricing: "free", requires: ["@stormeoio/auth"], envVars: {}, version: PACKAGE_VERSION, category: "compliance" },
  { id: "@stormeoio/crm", shortName: "crm", name: "CRM", description: "Contacts, organisations et pipeline commercial pour agences et SaaS", tags: ["crm", "contacts", "pipeline", "sales"], pricing: "free", requires: ["@stormeoio/auth"], envVars: {}, version: PACKAGE_VERSION, category: "business" },
  { id: "@stormeoio/ticketing", shortName: "ticketing", name: "Ticketing", description: "Support tickets, commentaires, labels et workflow de traitement", tags: ["ticketing", "support", "helpdesk"], pricing: "free", requires: ["@stormeoio/auth"], envVars: {}, version: PACKAGE_VERSION, category: "business" },
  { id: "@stormeoio/stripe", shortName: "stripe", name: "Stripe", description: "Paiements Stripe, webhooks, abonnements et gestion des clients", tags: ["stripe", "payments", "billing", "webhooks"], pricing: "free", requires: ["@stormeoio/auth"], envVars: { STRIPE_SECRET_KEY: { description: "Stripe secret key", required: true, example: "sk_test_..." }, STRIPE_WEBHOOK_SECRET: { description: "Stripe webhook secret", required: true, example: "whsec_..." } }, version: PACKAGE_VERSION, category: "payments" },
  { id: "@stormeoio/billing", shortName: "billing", name: "Billing", description: "Facturation, abonnements récurrents, devis et avoirs", tags: ["billing", "invoicing", "subscriptions"], pricing: "free", requires: ["@stormeoio/auth"], envVars: {}, version: PACKAGE_VERSION, category: "payments" },
  { id: "@stormeoio/cms", shortName: "cms", name: "CMS", description: "Gestion de contenu, pages, articles et SEO", tags: ["cms", "content", "pages", "seo"], pricing: "free", requires: ["@stormeoio/auth"], envVars: {}, version: PACKAGE_VERSION, category: "content" },
  { id: "@stormeoio/messaging", shortName: "messaging", name: "Messaging", description: "Messagerie interne, emails transactionnels et notifications", tags: ["messaging", "email", "notifications", "im"], pricing: "free", requires: ["@stormeoio/auth"], envVars: {}, version: PACKAGE_VERSION, category: "communication" },
  { id: "@stormeoio/drive", shortName: "drive", name: "Drive", description: "Stockage de fichiers, GED et partage sécurisé", tags: ["drive", "files", "storage", "ged"], pricing: "free", requires: ["@stormeoio/auth"], envVars: {}, version: PACKAGE_VERSION, category: "content" },
  { id: "@stormeoio/monitoring", shortName: "monitoring", name: "Monitoring", description: "Uptime monitoring, health checks et alertes", tags: ["monitoring", "uptime", "health", "alerts"], pricing: "free", requires: ["@stormeoio/auth"], envVars: {}, version: PACKAGE_VERSION, category: "devops" },
];

const AVAILABLE_IDS = new Set(["@stormeoio/auth", "@stormeoio/auth-social", "@stormeoio/consent", "@stormeoio/crm", "@stormeoio/ticketing", "@stormeoio/stripe"]);
